...
docker compose exec db pg_dump -U postgres -d postgres -Fc > backup-$(date +%F).dump  # manual backup
```

#### Daily backups to Cloudflare R2

The `backup` service (`docker/backup/`) runs `pg_dump` on start and then every day at `BACKUP_HOUR` (UTC; default 01:00, 04:00 in Kampala). It encrypts each dump with AES-256, uploads it to `R2_BUCKET/deathspot/db/YYYY/MM/`, and deletes copies older than `BACKUP_RETENTION_DAYS` (default 30).

1. In Cloudflare, create an R2 bucket (e.g. `deathspot-backups`), then **R2 > Manage API tokens > Create** a token with *Object Read & Write* on that bucket only.
2. Set `R2_ACCOUNT_ID`, `R2_BUCKET`, `R2_ACCESS_KEY_ID` and `R2_SECRET_ACCESS_KEY`. Keep `BACKUP_ENCRYPTION_KEY` (generated by `setup-env.mjs`) in a password manager as well: without it the backups can't be restored.
3. On Coolify the service always runs; with `docker-compose.yml`, add `--profile backup`.

```bash
docker compose logs backup                                   # "[backup] ... uploaded deathspot-....dump.enc"
docker compose exec backup backup.sh once                    # back up now
docker compose exec backup backup.sh list                    # list backups in R2
